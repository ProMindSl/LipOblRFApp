//
//  NavigationExtensions.swift
//  LipOblRFApp
//
//  Created by Viatcheslav Avdeev on 14/08/2019.
//  Copyright © 2019 Viatcheslav Avdeev. All rights reserved.
//

import Foundation
import UIKit
extension UIStoryboard{
    public static let VIEW_TYPE_NEWS_LIST = 0
    public static let VIEW_TYPE_NAV_MENU = 3
    public static let VIEW_TYPE_IDEAS_LIST = 1
    public static let VIEW_TYPE_LOGIN = 2
    public static let VIEW_TYPE_ADD_IDEA_FORM = 4
    public static let VIEW_TYPE_IDEACLIME_MENU = 5
    public static let VIEW_TYPE_NEWS_DETAIL = 6
    public static let VIEW_TYPE_ADD_CLAIM_FORM = 7
    public static let VIEW_TYPE_ADD_IDEA_STEP_1 = 8
    public static let VIEW_TYPE_ADD_IDEA_STEP_2 = 9
    public static let VIEW_TYPE_ADD_IDEA_STEP_3 = 10
    public static let VIEW_TYPE_ADD_IDEA_STEP_FINAL = 11
}

